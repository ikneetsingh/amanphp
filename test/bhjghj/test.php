...
<?php
$str2=mysql_fetch_array(mysql_query("select * from tbl_subjects where s_sno='$str1[s_subjects]'"));
				?>
                <option disabled="disabled" selected="selected">--select--</option>
                <?php 
						while($sub=mysql_fetch_array($str))
						{
							
							echo "<option value='$sub[s_sno]'";
							if($str2['s_sno']==$str['s_subjects'])
								echo "selected=selected>$sub[s_subjects]";
...
{
	echo "<option value='$row[s_sno]'>$row[s_subjects]</option>";
}
?>
</select></td></tr>
 <tr>
